...
TEST_F(SinksApiTests, RemoteSinkParameters) {
    ...
    for(const auto& ip: correctIPv4s){
        EXPECT_NO_THROW(RemoteSink::Params(ip, correctPort));
    }
    for(const auto& ip: incorrectIPv4s){
...
TEST_F(SinksApiTests, RemoteSinkCreation) {
    ...
    params.verbosity = Logger::Verbosity::Error;
...
TEST_F(SinksApiTests, ConsoleSinkCreation) {
    ...
    params.verbosity = Logger::Verbosity::Debug;
...
TEST_F(SinksApiTests, FileSinkParameters){
    ...
    if(exists(incorrectPath)){
        if (!hasWriteAccess(incorrectPath)) {
            EXPECT_ANY_THROW(FileSink({incorrectPath, correctFileName}));
        }else{
            std::cerr << "[warning] Program has access to /var folder, cannot test access rights." << std::endl;
        }
    }else{
        std::cerr << "[warning] Unable to test existing file without access (/var does not exists)." << std::endl;
    }
    ...
    if(exists(nonExistingPath)){
        std::cerr << "[warning] Unable to test non-existing folder." << std::endl;
    }else{
...
TEST_F(SinksApiTests, FileSinkCreation){
    ...
    params.maxFileSize = 56666;
    params.numberOfRotatedFiles = 5;
