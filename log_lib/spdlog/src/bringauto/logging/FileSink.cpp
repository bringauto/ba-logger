...
void bringauto::logging::FileSink::init(const std::string& logerName) {
    ...
    auto sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(params_.fileDir.string() + params_.fileName, params_.maxFileSize, params_.numberOfRotatedFiles, false);
