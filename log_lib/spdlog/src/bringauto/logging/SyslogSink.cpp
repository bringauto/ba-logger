...
const std::map<Option, int> SyslogSink::optionMap = {
...
const std::map<Facility, int> SyslogSink::facilityMap = {
...
void SyslogSink::init(const std::string &logerName) {
	...
	auto sink = std::make_shared<spdlog::sinks::syslog_sink_mt>(params_.identiy_, optionMap.at(params_.options_), facilityMap.at(params_.facility_), params_.enableFormatting_);
